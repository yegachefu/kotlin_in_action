...
println(sum(1,2))
//or
run{ println(42) } //실행 시점에 람다(익명함수)를 호출하는 것은 부가 비용이 들지 않는다.
```
...
val getAge = ::age //최상위 함수를 참조한다. 예를 들어서 클래스 외부에 선언한 함수의 경우.
//run 함수에 넘긴다.
```
...
//-> {a=[a,ab], b=[b]}
Q. groupBy는 원본을 건드리지 않고 slice는 원본을 건드리는데 왜 groupBy가 만들어졌을까?
내 생각에는 함수는 사이드이펙트를 만들지 않고 항상 동일한 결과를 뱉어야하기 때문에 그 이념에 맞춰서
원본을 건드리지 않기 위해 만든거같은데 실제는??
```
...
map 과 flapMap의 차이 
map + flatten = flatMap
[그림 5.6] + [코틀린에서 직접 해봤을 it 의 인자가 뭐로 나오는지 캡쳐]
```
